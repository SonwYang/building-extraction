...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes=1,  pretrained=False, use_batchnorm=True, freeze_encoder=False):
        ...
        self.center = ASPP(encoder_channels[0], decoder_channels[0])

        #deep supervision
        self.dsv1 = nn.Conv2d(out_channels[1], num_classes, kernel_size=1)
        self.dsv2 = nn.Conv2d(out_channels[2], num_classes, kernel_size=1)
        self.dsv3 = nn.Conv2d(out_channels[3], num_classes, kernel_size=1)
        self.dsv4 = nn.Conv2d(out_channels[3], num_classes, kernel_size=1)

        self.layer1 = DecoderBlock(in_channels[0], out_channels[1], use_batchnorm=use_batchnorm)
        self.layer2 = DecoderBlock(in_channels[1], out_channels[2], use_batchnorm=use_batchnorm)
        self.layer3 = DecoderBlock(in_channels[2], out_channels[3], use_batchnorm=use_batchnorm)
...
class ASPP(nn.Module):
    def __init__(self, inplanes=512, mid_c=256, dilations=[1, 3, 5, 7]):
