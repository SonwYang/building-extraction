...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes=1,  pretrained=False, use_batchnorm=True, freeze_encoder=False):
        ...
        self.dsv2 = nn.Sequential(
            _BoundaryRefineModule(out_channels[1]),
            nn.Conv2d(out_channels[1], num_classes, kernel_size=1)
        )
        self.dsv3 = nn.Sequential(
            _BoundaryRefineModule(out_channels[2]),
            nn.Conv2d(out_channels[2], num_classes, kernel_size=1)
        )
        self.dsv4 = nn.Sequential(
            _BoundaryRefineModule(out_channels[3]),
            nn.Conv2d(out_channels[3], num_classes, kernel_size=1)
        )
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(num_classes * 3, num_classes, kernel_size=1)
        )
