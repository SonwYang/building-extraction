...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels,
        ...
        self.block = nn.Sequential(
            ...
            _BoundaryRefineModule(out_channels)
        )
...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes=1,  pretrained=False, use_batchnorm=True, freeze_encoder=False):
        ...
        self.BR1 = _BoundaryRefineModule(encoder_channels[1])
        self.BR2 = _BoundaryRefineModule(encoder_channels[2])
        self.BR3 = _BoundaryRefineModule(encoder_channels[3])
        self.BR4 = _BoundaryRefineModule(encoder_channels[4])
    ...
    def forward(self, x):
        ...
        conv4 = self.up(conv4)
        conv3 = self.BR1(conv3)
        x = self.layer1([conv4, conv3])

        x = self.up(x)
        conv2 = self.BR2(conv2)
        x = self.layer2([x, conv2])

        x = self.up(x)
        conv1 = self.BR3(conv1)
        x = self.layer3([x, conv1])

        x = self.up(x)
        conv0 = self.BR4(conv0)
