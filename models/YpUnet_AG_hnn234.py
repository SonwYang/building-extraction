...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        conv01 = self.pool(conv0)
        conv1 = self.conv1(conv01)
        conv2 = self.conv2(conv1)
        conv3 = self.conv3(conv2)
        conv4 = self.conv4(conv3)
        print(f"conv0:{conv0.size()}")  # [32, 64, 256, 256]
        print(f"conv01:{conv01.size()}")    #[32, 64, 256, 256]
        ...
        print(f"x1:{x1.size()}")    #[32, 512, 16, 16]
        conv2 = self.AG2(x1, conv2)
        print(f"AG2:{conv2.size()}")    #[32, 512, 16, 16]
        x2 = self.layer2([x1, conv2])
        print(f"x2:{x2.size()}")    #[32, 512, 16, 16]
        conv1 = self.AG3(x2, conv1)
        print(f"AG3:{conv1.size()}")    #[32, 512, 16, 16]
        x3 = self.layer3([x2, conv1])
        print(f"x3:{x3.size()}")    #[32, 512, 16, 16]
        conv0 = self.AG4(x3, conv0)
        print(f"AG4:{conv0.size()}")    #[32, 512, 16, 16]
        x4 = self.layer4([x3, conv0])
        print(f"x4:{x4.size()}")    #[32, 512, 16, 16]

        #deep supervison
        size = x4.size()[2:]
        dsv1 = self.dsv1(F.upsample_bilinear(x1, size=size))
        dsv2 = self.dsv2(F.upsample_bilinear(x2, size=size))
        print(f"dsv2:{dsv2.size()}")    #[32, 512, 16, 16]

        dsv3 = self.dsv3(F.upsample_bilinear(x3, size=size))
        print(f"dsv3:{dsv3.size()}")    #[32, 512, 16, 16]

        dsv4 = self.dsv4(x4)
        print(f"dsv4:{dsv4.size()}")    #[32, 512, 16, 16]

        fuse = self.fuse(torch.cat([dsv2, dsv3, dsv4], dim=1))
        print(f"fuse:{fuse.size()}")    #[32, 512, 16, 16]
