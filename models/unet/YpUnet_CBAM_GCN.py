...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes=1,  pretrained=False, use_batchnorm=True, freeze_encoder=False):
        ...
        self.gcm1 = _GlobalConvModule(encoder_channels[1], encoder_channels[1], (7, 7))
        self.gcm2 = _GlobalConvModule(encoder_channels[2], encoder_channels[2], (7, 7))
        self.gcm3 = _GlobalConvModule(encoder_channels[3], encoder_channels[3], (7, 7))
        self.gcm4 = _GlobalConvModule(encoder_channels[4], encoder_channels[4], (7, 7))
    ...
    def forward(self, x):
        ...
        x = self.layer1([conv4, self.gcm1(conv3)])
        x = self.layer2([x, self.gcm2(conv2)])
        x = self.layer3([x, self.gcm3(conv1)])
        x = self.layer4([x, self.gcm4(conv0)])
