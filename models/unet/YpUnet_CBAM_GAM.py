...
class Golobal_attention_module(nn.Module):
    def __init__(self, channels_high, channels_low):
        super(Golobal_attention_module, self).__init__()
        ...
        self.fc1   = nn.Conv2d(channels_low, channels_low // 16, 1, bias=False)
        self.relu1 = nn.ReLU()
        self.fc2   = nn.Conv2d(channels_low // 16, channels_low, 1, bias=False)
    ...
    def forward(self, fms_high, fms_low):
        fms_high_gp = self.conv1x1(fms_high)
        fms_high_gp = self.bn_high(fms_high_gp)
        fms_high_gp = self.relu(fms_high_gp)
        avg_out = self.fc2(self.relu1(self.fc1(self.avg_pool(fms_high_gp))))
        max_out = self.fc2(self.relu1(self.fc1(self.max_pool(fms_high_gp))))
        fms_high_gp = self.sigmoid(avg_out + max_out)
...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes=1,  pretrained=False, use_batchnorm=True, freeze_encoder=False):
        ...
        self.GAM1 = Golobal_attention_module(encoder_channels[0], encoder_channels[1])
        self.GAM2 = Golobal_attention_module(out_channels[0], encoder_channels[2])
        self.GAM3 = Golobal_attention_module(out_channels[1], encoder_channels[3])
        self.GAM4 = Golobal_attention_module(out_channels[2], encoder_channels[4])
    ...
    def forward(self, x):
        ...
        conv3 = self.GAM1(conv4, conv3)
        x = self.layer1([conv4, conv3])

        conv2 = self.GAM2(x, conv2)
        x = self.layer2([x, conv2])

        conv1 = self.GAM3(x, conv1)
        x = self.layer3([x, conv1])

        conv0 = self.GAM4(x, conv0)
