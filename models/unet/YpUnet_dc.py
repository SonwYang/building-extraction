...
class dilatedConvModule(nn.Module):
    def __init__(self, in_channels, out_channels, dilations=3):
        super(dilatedConvModule, self).__init__()
        self.dc =  nn.Sequential(
                nn.Conv2d(in_channels, out_channels, kernel_size=3,
                                     stride=1, padding=dilations, dilation=dilations, bias=False),
...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes=1,  pretrained=False, use_batchnorm=True, freeze_encoder=False):
        ...
        self.dc1 = dilatedConvModule(encoder_channels[1], encoder_channels[1], dilations=2)
        self.dc2 = dilatedConvModule(encoder_channels[2], encoder_channels[2], dilations=3)
        self.dc3 = dilatedConvModule(encoder_channels[3], encoder_channels[3], dilations=5)
        self.dc4 = dilatedConvModule(encoder_channels[4], encoder_channels[4], dilations=7)
    ...
    def forward(self, x):
        ...
        conv3 = self.dc1(conv3)
        conv2 = self.dc2(conv2)
        conv1 = self.dc3(conv1)
        conv0 = self.dc4(conv0)
