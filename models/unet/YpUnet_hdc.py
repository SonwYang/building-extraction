...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes=1,  pretrained=False, use_batchnorm=True, freeze_encoder=False):
        ...
        self.center = nn.Sequential(
            nn.Conv2d(encoder_channels[0], encoder_channels[0], kernel_size=3,
            ...
            nn.BatchNorm2d(encoder_channels[0]),
            nn.ReLU(),
            nn.Conv2d(encoder_channels[0], 1024, kernel_size=3,
                      stride=1, padding=4, dilation=4, bias=False),
            nn.BatchNorm2d(1024),
            nn.ReLU(),
            nn.Conv2d(1024, encoder_channels[0], kernel_size=1), nn.BatchNorm2d(encoder_channels[0]),  nn.ReLU()
        )
